...
def getfinalurl_content(partlists,listpageurl,finalparseurl):
        print "args:",partlists,listpageurl,finalparseurl
        ...
        if listpageurl == 1:
            print "parseFinallyurl_rult:",parseFinallyurl_rult
...
#整体控制的run方法
def run():
    partlists={"http://list.jd.com/list.html?cat=737%2C794%2C870&go=0":'空调'}
...
#获取最后页面的具体内容
def test(finalparseurl):
    print "args:",finalparseurl
    parseFinallyurl_rult=parseFinallyurl(finalparseurl)
    htmlname_tmp=urlparse(finalparseurl).path
    imgtopdir_tmp="img/"+htmlname_tmp.split('/')[1][0:2]
    imgdir=imgtopdir_tmp+htmlname_tmp+"/introduction"
    ...
    #specification_tmp:商品的规则参数
    #typeOrsize_tmp:商品的类型和尺寸
    print parseFinallyurl_rult
    return finalparseurl.strip()+"\t"+str(parseFinallyurl_rult["title"]).strip()+"\t"+str(parseFinallyurl_rult["jdprice"]).strip()\
           +"\t"+str(parseFinallyurl_rult["refprice"]).strip()+"\t"+imgshowdir.strip()+"\t"+imgdir.strip()+"\t"+partdetail_tmp.strip()+"\t"+specification_tmp.strip()+"\t"+\
           typeOrsize_tmp.strip()
str1 = test("http://item.jd.com/10028634194.html")
...
if __name__ == '__main__':
    ...
    for link in judgeurl_all_lines:
        finalurl_content = getfinalurl_content({1:"##"},1,link)
        with open("data2.txt","a") as datafile:#将爬取完毕好的url写入data.txt
            datafile.writelines(finalurl_content+"\n")
